...
class _Zha:
    ...
    async def create_group(self, name: str, ieee: str) -> Any:
        ...
        gw = self._gateway()
        member = GroupMemberReference(
            ieee=self._eui64(ieee), endpoint_id=GROUP_MEMBER_ENDPOINT
        )
...
class ScenePairingEngine:
    ...
    _color_cache: dict[str, tuple[str | None, str | None]] = field(
    ...
    async def _hold(self, ieee: str) -> None:
        ...
        if self._active(ieee, is_anchor=False):  # window open, X is not the anchor
            ...
            if existing is not None and getattr(existing, "group_id", None) == gid:
                ...
                act = "joined"
            self._state = _State(anchor=anchor, group_id=gid, ts=self.hass.loop.time(),
                                 color=color, cidx=st.cidx)
            self._arm_timer()
        ...
        self._state = _State(anchor=ieee, group_id=gid, ts=self.hass.loop.time(),
                             color=color, cidx=cidx)
        if gid is None:
        ...
        for entity_id in (on, off):
            if entity_id:
                await self.hass.services.async_call(
                    "number", "set_value",
                    {"entity_id": entity_id, "value": hue}, blocking=True,
                )
    ...
    async def _issue_led_effect(
        ...
        await self.hass.services.async_call(
            ...
            {
                "ieee": ieee, "endpoint_id": 1,
                "cluster_id": INOVELLI_MFG_CLUSTER, "cluster_type": "in",
                "command": LED_EFFECT_CMD, "command_type": "server",
                "manufacturer": INOVELLI_MFG_ID,
                "params": {
                    "led_effect": effect, "led_color": color,
                    "led_level": level, "led_duration": duration,
                },
            },
            blocking=True,
        )
